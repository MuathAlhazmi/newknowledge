...
import { HeaderCourseSearchSlot } from "@/components/header-course-search-slot";
...
export async function SiteHeader({
  ...
  const showCourseSearchSlot =
    Boolean(user?.role === UserRole.PARTICIPANT && user.platformApproved);

  return (
    <header className="sticky top-0 z-40 bg-transparent">
      <div className="mx-auto max-w-6xl px-4 pt-3 pb-2 md:px-6 md:pt-3.5 md:pb-3">
        <div className="nk-header-inner flex flex-col overflow-visible">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3 px-4 py-3 md:px-5 md:py-3.5">
            <Link
              href={brandHref}
          ...
          <HeaderCourseSearchSlot enabled={showCourseSearchSlot} />
        </div>
      </div>
    </header>
  );
